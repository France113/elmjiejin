...
async function searchContent(keyword) {
    try {
        ...
        const url = `https://res.yunpan.win/?PageIndex=1&PageSize=30&Keyword=${encodedKeyword}&YunPanSourceType=2&Type=&Tag=`;
        const html = await 访问网页(url);
        console.log(html);
        const items = 文本_取中间_批量(html,'<div class="col">','详情</a>');
        const list = items.map((item) => {
            const vod_name = 文本_取中间(item,'夸克云盘</span>','</h5>');
            const decodedString = vod_name.replace(/&#x([\da-fA-F]+);/g, (match, p1) => String.fromCharCode(parseInt(p1, 16)));
            const vod_pic ='https://res.yunpan.win' +  文本_取中间(item,'<img src="','"') ;
            const vod_id = 'https://res.yunpan.win' + 文本_取中间(文本_取中间(item,'<a href="javascript:void(0)" class="card-link"','class="card-link">'),'<a href="','"');
            const vod_remarks = '';
            return {
                vod_id: vod_id,
                vod_name: decodedString,
...
async function detailContent(ids) {
  ...
  try {
    ...
    // 使用正则表达式提取信息
    const vod_id = ids;
    const encodedString = 文本_取中间(html, '夸克云盘</span>', '</h5>');
    const vod_name = encodedString.replace(/&#x([\da-fA-F]+);/g, (match, p1) => String.fromCharCode(parseInt(p1, 16)));
    ...
    const vod_pic = 'https://res.yunpan.win'+文本_取中间(html, '<img src="', '"');
    //console.log(vod_pic);
    let vod_remarks='';
     //console.log(vod_remarks);
    const encodedString2 = 文本_取中间(html, '<p class="card-text" style="white-space:pre-line;">', '</p>');
    const vod_content = encodedString2.replace(/&#x([\da-fA-F]+);/g, (match, p1) => String.fromCharCode(parseInt(p1, 16)));
    //console.log(vod_content);
    const cloudLinks = 文本_取中间(html, "window.open('", "')") || '';
